...
//------------------------------------------------------------------------------------------------
// This function gets the user id after checking if the user is logged in, and passes it to the keep the button selected function
//------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------
// This function displays the components of each post, by taking the field values from data stores
...
// of the post document by 1. Increment occurs when the post does not exist in the viewposthistory array, otherwise, decrement occurs.
//-----------------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------
// This function keeps the add post to history button selected if the page is reloaded
...
function keepAddToHistoryButtonSelected() {
    userDocRef.get().then(userDoc => {
        ...
        // If the postviewhistory field is not null the current post if in the postviewhistory array includes the id of the post document
        if (postViewHistory && postViewHistory.includes(clickedPostID)) {
            let removePostMsg = "Remove this post from history";
            document.getElementById("add-post-to-history-button").innerHTML = removePostMsg;
